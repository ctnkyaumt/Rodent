...
{
    // Every discovered device is reported, whatever the brand. Each block below
    // asks for the capability it needs (see Rodent.Core/Devices/Capabilities.cs)
    // and says so when this driver hasn't got there yet — nothing is skipped.
    Console.WriteLine($"== {dev.Name}  ({dev.Brand}, {dev.Kind}, {dev.VendorId:X4}:{dev.ProductId:X4}, support: {dev.Support})");
    Console.WriteLine($"   capabilities: {Caps(dev)}");
    if (dev.Firmware is { Length: > 0 } fw) Console.WriteLine($"   - Firmware: {fw}");
    foreach (var info in dev.Info) Console.WriteLine($"   - {info.Label}: {info.Value}");
    foreach (var s in dev.Settings)
    ...
    var buttons = dev as IButtonDevice;
    var macros = dev as IMacroDevice;
    var lighting = dev as ILightingDevice;
    if (buttons != null)
        foreach (var b in buttons.Buttons) Console.WriteLine($"   button {b.Index}: {b.Label}");

    // HID++-only experiments below need raw feature access; d is non-null exactly
    // when this driver speaks HID++ 2.0.
    var d = dev as IHidppDevice;
    bool NeedsHidpp(string command)
    {
        if (d != null) return false;
        Console.WriteLine($"   ({command}: needs raw HID++ access — {dev.Brand} driver doesn't expose it)");
        return true;
    }

    if (args.Contains("writetest"))
    {
        if (buttons == null || buttons.Buttons.Count < 7)
            Console.WriteLine("   (writetest: this device doesn't expose 7+ remappable buttons)");
        else
        {
            const int idx = 7; // Win+Tab on the G402
            byte[]? orig = buttons.ReadButtonBytes(idx);
            Console.WriteLine($"\n   WRITE TEST on button {idx} (orig bytes: {Hex(orig)})");

            var (ok, label) = buttons.RemapButton(idx, new byte[] { 0x80, 0x01, 0x00, 0x04 }); // Middle Click
            Console.WriteLine($"   wrote Middle Click -> ok={ok}, reads back: {label}");

            if (orig != null)
            {
                var (ok2, label2) = buttons.RemapButton(idx, orig);
                Console.WriteLine($"   restored original -> ok={ok2}, reads back: {label2}");
            }
        }
    }

    if (args.Contains("mode") && !NeedsHidpp("mode"))
    {
    ...
    if (args.Contains("leds") && !NeedsHidpp("leds"))
    {
    ...
    if (args.Contains("ledwrite") && !NeedsHidpp("ledwrite"))
    {
    ...
    if (args.Contains("ledwrite2") && !NeedsHidpp("ledwrite2"))
    {
        HidppTransport.Debug = true;
    ...
    if (args.Contains("ledwrite3") && !NeedsHidpp("ledwrite3"))
    {
    ...
    if (args.Contains("dumpprofile") && !NeedsHidpp("dumpprofile"))
    {
    ...
    if (args.Contains("ledsw") && !NeedsHidpp("ledsw"))
    {
    ...
    if (args.Contains("ledfix") && !NeedsHidpp("ledfix"))
    {
    ...
    if (args.Contains("ledghub") && !NeedsHidpp("ledghub"))
    {
    ...
    if (args.Contains("ledbright") && !NeedsHidpp("ledbright"))
    {
    ...
    if (args.Contains("ledmodes") && !NeedsHidpp("ledmodes"))
    {
        string Sample(int idx)
        {
            var vals = new List<string>();
    ...
    if (args.Contains("ledapp") && !NeedsHidpp("ledapp") && lighting != null)
    {
        ...
        void Run(string name, ProfileEdit.LightingConfig cfg)
            => Console.WriteLine($"   {name}: write={lighting.WriteLighting(cfg)} logo=[{Sample()}]");
    ...
    if (args.Contains("strip") && !NeedsHidpp("strip"))
    {
    ...
    if (args.Contains("fadetest") && !NeedsHidpp("fadetest"))
    {
    ...
    if (args.Contains("fade2") && !NeedsHidpp("fade2") && lighting != null)
    {
        string Sample() { var v = new List<string>(); for (int i = 0; i < 5; i++) { var s = d!.Features.Call(0x1300, 0x40, 0x01); v.Add(s == null ? "??" : $"{s[3]:X2}"); Thread.Sleep(300); } return string.Join(" ", v); }
        ProfileEdit.LightingConfig C(int mode, byte shade) => new(mode, 0, shade, shade, 3000);
        lighting.WriteLighting(C(ProfileEdit.LedFixed, 0xFF));
        Thread.Sleep(2500);
        Console.WriteLine($"   bright (settled): {Sample()}");
        lighting.WriteLighting(C(ProfileEdit.LedFixed, 0x60));                       // persists (flash)
        Console.WriteLine($"   -> dim, persist:  {Sample()}");
        Thread.Sleep(1500);
        lighting.WriteLighting(C(ProfileEdit.LedFixed, 0xFF), persist: false);       // per-app path
        Console.WriteLine($"   -> bright, live:  {Sample()}");
    }

    if (args.Contains("ledread") && !NeedsHidpp("ledread"))
    {
    ...
    if (args.Contains("fwtest") && !NeedsHidpp("fwtest") && lighting != null)
    {
        ...
        lighting.WriteLighting(new ProfileEdit.LightingConfig(ProfileEdit.LedFixed, 0, 0x60, 0x60, 3000));
        Thread.Sleep(2000);
        Console.WriteLine($"   software fixed:  {Sample(4)}");
        lighting.WriteLighting(new ProfileEdit.LightingConfig(ProfileEdit.LedOff, 0, 0, 0, 3000, FirmwareMode: true));
    ...
    if (args.Contains("repeatwrite") && !NeedsHidpp("repeatwrite") && buttons != null && macros != null && buttons.Buttons.Count >= 8)
    {
        const int idx = 8;
        ...
        var (ok, sec, addr, err) = macros.AssignMacro(idx, steps, Macro.RepeatMode.WhileHeld);
        ...
        if (sec != null && addr != null)
        {
            ...
            Console.WriteLine($"   label:  {buttons.Buttons.First(b => b.Index == idx).Label}");
        }
        if (ms != null && origSec != null) OnboardProfiles.WriteRawSector(d!.Features, ms.Value, origSec);
        if (origBtn != null) buttons.RemapButton(idx, origBtn);
        Console.WriteLine($"   restored button {idx}");
    }

    if (args.Contains("stripe") && !NeedsHidpp("stripe"))
    {
    ...
    if (args.Contains("dumpall") && !NeedsHidpp("dumpall"))
    {
        for (int s = 0; s <= 2; s++)
        {
    ...
    if (args.Contains("stripeon") && !NeedsHidpp("stripeon"))
    {
    ...
    if (args.Contains("features") && !NeedsHidpp("features"))
    {
    ...
    if (args.Contains("stripeon2") && !NeedsHidpp("stripeon2"))
    {
        string St() { var s = d!.Features.Call(0x1300, 0x40, 0x00); return s == null ? "??" : Hex(s[..4]); }
    ...
    if (args.Contains("stripeon3") && !NeedsHidpp("stripeon3"))
    {
    ...
    if (args.Contains("stripeon4") && !NeedsHidpp("stripeon4"))
    {
        d!.Features.Call(0x1E00, 0x10, 0x01);
        string St() { var s = d!.Features.Call(0x1300, 0x40, 0x00); return s == null ? "??" : Hex(s[..4]); }
        void T(string name, byte fn, params byte[] p)
    ...
    if (args.Contains("nvread") && !NeedsHidpp("nvread"))
    {
        d!.Features.Call(0x1E00, 0x10, 0x01);
    ...
    if (args.Contains("nvwrite") && !NeedsHidpp("nvwrite"))
    {
        d!.Features.Call(0x1E00, 0x10, 0x01);
    ...
    if (args.Contains("nvapp") && !NeedsHidpp("nvapp") && lighting != null)
    {
        ...
        Console.WriteLine($"   indicator: write={lighting.WriteLighting(fwOff)} nv={Nv()}  (want 00 04)");
        Thread.Sleep(1500);
        Console.WriteLine($"   always-on: write={lighting.WriteLighting(fwOn)} nv={Nv()}  (want 00 02)");
    }

    if (args.Contains("readmacros") && !NeedsHidpp("readmacros"))
    {
        int? ms = OnboardProfiles.FindMacroSector(d!.Features);
    ...
    if (args.Contains("sectors") && !NeedsHidpp("sectors"))
    {
    ...
    if (args.Contains("repeattest") && !NeedsHidpp("repeattest") && buttons != null && macros != null && buttons.Buttons.Count >= 8)
    {
        const int idx = 8;
        ...
        var (ok, sec, addr, _) = macros.AssignMacro(idx, steps, Macro.RepeatMode.WhileHeld);
        ...
        if (origBtn != null) buttons.RemapButton(idx, origBtn);
        Console.WriteLine($"   restored button {idx} -> {buttons.Buttons.First(b => b.Index == idx).Label}");
    }

    if (args.Contains("macrotest") && !NeedsHidpp("macrotest") && buttons != null && macros != null && buttons.Buttons.Count >= 8)
    {
        ...
        var (ok, sector, address, _) = macros.AssignMacro(idx, steps);
        ...
        Console.WriteLine($"   button {idx} now reads: {buttons.Buttons.First(b => b.Index == idx).Label}");
...
/// <summary>What this driver can do, so a brand's progress is visible at a glance.</summary>
static string Caps(IDeviceDriver d)
{
    var caps = new List<string>();
    if (d is IButtonDevice) caps.Add("buttons");
    if (d is IMacroDevice) caps.Add("macros");
    if (d is ILightingDevice) caps.Add("lighting");
    if (d is IDpiDevice) caps.Add("dpi");
    if (d is IHidppDevice) caps.Add("hid++");
    if (d.Settings.Count > 0) caps.Add($"{d.Settings.Count} setting(s)");
    return caps.Count == 0 ? "identification only" : string.Join(", ", caps);
}
